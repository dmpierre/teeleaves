...
if [[ $2 == "-f" || $2 == "--debug" ]]; then
  ...
  docker build -t teeleaves .
fi

# Create the EIF from the enclave.
nitro-cli build-enclave --docker-uri teeleaves:latest --output-file teeleaves.eif
...
RUN_COMMAND="nitro-cli run-enclave --cpu-count $ENCLAVE_CPU_COUNT --memory $ENCLAVE_MEMORY --eif-path teeleaves.eif --enclave-cid $ENCLAVE_CID"
...
if [[ $1 == "run" ]]; then
  ...
  if [[ $2 == "-f" ]]; then
    nitro-cli console --enclave-name teeleaves
  fi
fi
